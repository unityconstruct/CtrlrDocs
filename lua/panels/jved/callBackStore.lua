...
--### This function call for a refresh a patch data on editor to be stored ###
...
callBackStore = function()
    ...
    -- Ask an user if want store patch and shows the place to store

    
    storeOption = panel:getModulatorByName("STORE-PLACE"):getModulatorValue()


    
    -- Test if the option is Internal


    if storeOption==0 then
    
        local x=utils.questionWindow("Save Patch?", "The selected patch on user memory will be overwritten!", "Yes", "No")

        if x==true then

            --Request all Patch Data from JV-880
            ...
            --Start Timer for security to get all patch data from refresh and calls function timerStoreCallback

            timer:setCallback (1, timerStoreCallback) --calls the function storePatch
            timer:startTimer(1, 2500) -- 2500 ms		
    ...
    -- Test if the option is Data Card


    if storeOption==1 then
    
        local x=utils.questionWindow("Save Patch?", "The selected patch on Data Card will be overwritten!", "Yes", "No")
    ...
    -- Test if the option is SAVE SYSEX

    
    if storeOption==2 then

        local x=utils.questionWindow("Save Patch?", "The patch will be saved on computer", "Yes", "No")
    ...
    -- Test if the option is LOAD SYSEX

    if storeOption==3 then
